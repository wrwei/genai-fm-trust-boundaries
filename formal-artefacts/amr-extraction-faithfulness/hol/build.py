"""Build the standalone AMR extraction session; preserve prior build attempts."""
import os
...
cmd = [str(INSTALL/'contrib/cygwin/bin/bash.exe'), '--noprofile', '--norc', '-c',
       ...
       'export PATH=/usr/bin:/bin:$PATH; exec "$@"', 'extraction-hol',
       cyg(INSTALL/'bin/isabelle'), 'build', '-e', '-D', cyg(BASE), '-v', 'AMR_Extraction_Faithfulness']
