...
class SourceSupervisor:
    def __init__(self, text):
        ...
        if not self.assessment['accepted']:
            raise ValueError('Source candidate rejected before deployment: ' + self.assessment['source_sha256'])
...
class HandwrittenSupervisor:
    ...
    identity = 'handwritten-obligation-reference/v1'
    assessment = None
            ...
            for name in robots:
                ...
                if action=='Brake' and facts['PedestrianBlocked'] and last_sent[name]!='Brake':
                    ...
                    record('pedestrian_brake',t,robot=name,observation_distance_to_H=distance,
                           required_distance=required,straight_approach_condition=distance>=required)
