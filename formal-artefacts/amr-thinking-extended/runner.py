"""One-shot, extended-output thinking diagnostic. No retry or resume path."""
...
PLAN = ROOT / 'literature/scholar_search_2026-09-10/followup/research_amr_extended_protocol_2026-09-17.md'
...
smoke = SMOKE = _load('_amr_extended_smoke', HERE.parent / 'amr-deepseek-smoke/smoke.py')
d = _load('_amr_extended_diagnostics', FORGE / 'diagnostics.py')

write_json = smoke.write_json
CONDITIONS = [('N64-F', 12, 'enhanced'), ('T64-F', 12, 'enhanced')]
MAX_OUTPUT = 65536
CONFIGS = {
    'N64-F': {'model': smoke.MODEL, 'max_tokens': MAX_OUTPUT, 'stream': False,
              'thinking': {'type': 'disabled'}, 'temperature': 0},
    'T64-F': {'model': smoke.MODEL, 'max_tokens': MAX_OUTPUT, 'stream': False,
...
def verify_inputs(run):
    ...
    if inventory(THINKING_RUN) != read_json(run/'thinking-inventory.json'):
        raise RuntimeError('historical thinking run changed')
    for rel, expected in read_json(run/'thinking-frozen-inputs.json').items():
        if sha(ROOT/rel) != expected:
            raise RuntimeError('historical thinking dependency changed: '+rel)
...
def initialize(run, fixture):
    ...
    thinking_before = inventory(THINKING_RUN)
    thinking_expected = read_json(THINKING_RUN/'evidence-sha256.json')
    if thinking_expected != {k:v for k,v in thinking_before.items() if k != 'evidence-sha256.json'}:
        raise RuntimeError('historical thinking evidence failed hash verification')
    thinking_frozen = read_json(THINKING_RUN/'frozen-inputs.json')
    for rel, expected in thinking_frozen.items():
        if sha(ROOT/rel) != expected:
            raise RuntimeError('historical thinking dependency changed: '+rel)
    ...
    paths |= {ROOT/rel for rel in thinking_frozen}
    ...
    write_json(run/'thinking-inventory.json', thinking_before)
    write_json(run/'thinking-frozen-inputs.json', thinking_frozen)
    (run/'starting-response.bin').write_bytes(START.read_bytes())
    write_json(run/'protocol.json', {
        'id': 'amr-thinking-extended/v1', 'mode': 'offline-fixture' if fixture else 'live',
        ...
        'client_wall_deadline_seconds':660, 'socket_timeout_seconds':600, 'source_spec': 'amr-supervisor-obligations/v1.1',
        'seed': {'sample_id':'P2-R1', 'old_sequence':4, 'sha256':sha(START)},
        'maximum_payload_bytes':MAX_PAYLOAD_BYTES,
        'physics_selection':'first accepted condition in N64-F,T64-F order, first accepted repair; exploratory only',
        'physics_cases':['normal_A','normal_B','temporary','blackout','permanent']})
    write_json(run/'setup-sha256.json', {n:sha(run/n) for n in
               ('frozen-inputs.json','prior-inventory.json','forge-inventory.json',
                'thinking-inventory.json','thinking-frozen-inputs.json','starting-response.bin','protocol.json')})
...
def execute(key_path, *, run=None, worker=run_worker, fixture=False):
    ...
    if stop != 'protocol_complete' or any(s['stop'] is None for s in states.values()):
        selected = None
    result = {'protocol_id':'amr-thinking-extended/v1', 'mode':'offline-fixture' if fixture else 'live',
