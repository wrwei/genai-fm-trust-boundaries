"""One-shot, four-condition diagnostic. No retry or resume path."""
...
PLAN = ROOT / 'literature/scholar_search_2026-09-10/followup/research_amr_forge_revision_2026-09-17.md'
...
CONDITIONS = [('D8-O', 8, 'original'), ('D12-O', 12, 'original'),
              ('D8-F', 8, 'enhanced'), ('D12-F', 12, 'enhanced')]
MAX_REPAIRS = 4
MAX_CALLS = 16
...
def initial_states():
    ...
    reports = {limit: d.assess(text, limit) for limit in (8, 12)}
...
def initialize(run, fixture):
    ...
    run.mkdir()
    (run/'attempts').mkdir()
    paths = set((HERE/'profiles').rglob('*.py')) | {
        HERE/'runner.py', HERE/'diagnostics.py', HERE/'README.md', HERE/'profiles/snapshot.json',
    ...
    write_json(run/'protocol.json', {
        'id': 'amr-forge-diagnostic/v1', 'mode': 'offline-fixture' if fixture else 'live',
        ...
        'model_config': smoke.CONFIG, 'source_spec': 'amr-supervisor-obligations/v1.1',
        'seed': {'sample_id':'P2-R1', 'old_sequence':4, 'sha256':sha(START)},
        'maximum_payload_bytes':MAX_PAYLOAD_BYTES,
        'physics_selection':'first accepted condition in D8-O,D12-O,D8-F,D12-F order, first accepted repair; exploratory only',
        'physics_cases':['normal_A','normal_B','temporary','blackout','permanent']})
    write_json(run/'setup-sha256.json', {n:sha(run/n) for n in
               ('frozen-inputs.json','prior-inventory.json','starting-response.bin','protocol.json')})
...
def network_child(key_path, attempt):
    ...
    payload = smoke.payload_for(prompt)
...
def decode_response(attempt, transport, payload):
    ...
    if (attempt/'http-body.bin').exists():
        try:
            ...
            usage_summary = smoke.usage_summary(envelope.get('usage'))
    ...
    if fatal is None:
        if usage_summary is None:
            fatal = 'invalid_usage'
        elif usage_summary['output_tokens'] > smoke.MAX_OUTPUT:
...
def execute(key_path, *, run=None, worker=run_worker, fixture=False):
    ...
    for _ in range(MAX_CALLS):
        ...
        outcome = {'condition':cid,'repair':repair,'started_at_utc':started,'finished_at_utc':finished,
                   ...
                   'provider_request_id':envelope.get('id'), 'files_sha256':inventory(folder)}
    ...
    if stop != 'protocol_complete' or any(s['stop'] is None for s in states.values()):
        selected = None
    result = {'protocol_id':'amr-forge-diagnostic/v1', 'mode':'offline-fixture' if fixture else 'live',
              ...
              'scope':'four repair trajectories from one previously observed failure; not independent task samples'}
