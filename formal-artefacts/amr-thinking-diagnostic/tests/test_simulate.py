...
class PhysicsTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if (HERE/'simulate.py').exists():
            spec = importlib.util.spec_from_file_location('thinking_physics_test', HERE/'simulate.py')
            cls.sim = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(cls.sim)

    def setUp(self):
        self.assertIsNotNone(self.sim, 'thinking physics adapter is absent')
    ...
    def test_selection_requires_both_terminal_conditions_and_fixed_order(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(self.sim.selected_record(result)['condition'],'N12-F')
            for change in ('missing','pending','wrong-selection','wrong-repair'):
                obj=deepcopy(result)
                if change=='missing': del obj['conditions']['T12-F']
                elif change=='pending': obj['conditions']['T12-F']['stop']=None
                elif change=='wrong-selection': obj['selected_for_exploratory_physics']['condition']='T12-F'
