...
SPEC_ID = 'amr-supervisor-obligations/v1.1'
...
def obligation(mode, f):
    ...
    goal_brake = not blocked and f['AtGoal'] and f['Released'] and not f['Halted']
    finish = not (blocked or goal_brake) and f['AtGoal'] and f['Halted'] and f['Released']
    release = not (blocked or goal_brake or finish) and f['OwnReservation'] and f['BodyClearOfZ']
    braking = not (blocked or goal_brake or finish or release) and mode == 'BrakeRequested' and not f['Halted']
    request = not (blocked or goal_brake or finish or release or braking) and not f['OwnReservation'] and not f['Released']
    resume = not (blocked or goal_brake or finish or release or braking or request) and mode == 'Stopped'
    requirements = (
        (blocked, 'stop_on_invalid_or_blocked_input', ('Brake', 'Stopped' if f['Halted'] else 'BrakeRequested')),
        (goal_brake, 'brake_at_released_goal_until_halted', ('Brake', 'BrakeRequested')),
        ...
        (not any((blocked, goal_brake, finish, release, braking, request, resume)), 'proceed_when_currently_allowed', ('Proceed', 'Traversing')),
    )
