...
Review clarification: the original scenario already requires that the reservation holder can exit and that no permanent hardware fault exists (`research_amr_scenario_2026-09-15.md`, section 7). The countermodel removes that qualitative progress premise; it is not a counterexample to the complete original premise set. The contribution is an explicit quantitative discharge obligation and a witness of its necessity. The pre-run version of this design is retained verbatim under the run's `inputs/` snapshot.

## Deliverables and acceptance

1. A kernel-checked parametric travel-time inequality for the analytic rest-to-rest phase profile. For acceleration `a>0`, braking `b>0`, speed cap `v>0`, define `k=1/(2a)+1/(2b)`. A segment of length `L` has duration at most `L/v+k*v`, provided its peak/cruise phases obey the exact profile equations. A finite phase schedule drains in its summed duration. These are mathematical profile theorems, not a mechanized refinement of Python floating-point integration.
